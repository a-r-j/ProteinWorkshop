...
from src.datasets.components.atom3d_dataset import BaseTransform
from torch.utils.data import IterableDataset
from torch_geometric.data import Data

PPI_DF_INDEX_COLUMNS = ["ensemble", "subunit", "structure", "model", "chain", "residue"]
...
@beartype
def _get_idx_to_res_mapping(df: pd.DataFrame) -> Tuple[pd.DataFrame, pd.Series]:
    """Define mapping from residue index to single id number."""
    # Adapted from: https://github.com/drorlab/atom3d/blob/master/examples/ppi/dataset/neighbors.py
    idx_to_res = get_res(df).reset_index(drop=True)
    res_to_idx = idx_to_res.reset_index().set_index(PPI_DF_INDEX_COLUMNS)["index"]
...
@beartype
def get_negatives(neighbors, df0: pd.DataFrame, df1: pd.DataFrame) -> pd.DataFrame:
        ...
        with torch.no_grad():
            for idx in indices:
                ...
                for ensemble_name, target_df in pairs.groupby(["ensemble"]):
                    sub_names, (bound1, bound2, unbound1, unbound2) = get_subunits(
                        target_df
                    )
