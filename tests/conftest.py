...
def cfg_train_global() -> DictConfig:
    ...
    with initialize(version_base="1.3", config_path="../configs"):
        ...
        with open_dict(cfg):
            cfg.env.paths.root_dir = str(pyrootutils.find_root(indicator=".project-root"))
...
def cfg_finetune_global() -> DictConfig:
    ...
    """
    with initialize(version_base="1.3", config_path="../configs"):
        cfg = compose(config_name="finetune.yaml", return_hydra_config=True, overrides=["ckpt_path=."])
...
def cfg_finetune(cfg_finetune_global: DictConfig, tmp_path: Path) -> DictConfig:
    """A pytest fixture built on top of the `cfg_finetune_global()` fixture, which accepts a temporary
    logging path `tmp_path` for generating a temporary logging path.

    This is called by each test which uses the `cfg_finetune` arg. Each test generates its own temporary logging path.
